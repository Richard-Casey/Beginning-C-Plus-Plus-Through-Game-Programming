...
int main()
{
	...
	// Her is our clock for timing everything
	Clock clock;
	...
	while (window.isOpen())
	{
		...
		if (state == State::LEVELING_UP)
		{
			...
			if (state == State::PLAYING)
			{
				...
				// Pass the vertex array by reference to the creat background function
				...
				// Delete the previously allocated emmory (if it exists)
				delete[] zombies;
				zombies = createHorde(numZombies, arena);
				numZombiesAlive = numZombies;

				// Reset the clock so there isnt frame jump
				clock.restart();
			}
		} // End levelling up
